...
void LDR_read (int);
int ultrasonido_1 (void);
int ultrasonido_2 (void);
...
void loop()
{
  ...
  if (mode == on) system_on(LDR_output);
...
void system_on (int z)
{
  ...
  else if (current_state == night)
  {
    ...
    if (((x > 20) and (y > 20)) and (blocker_motion_x == false))
    {
      analogWrite(DIMMER, DIMMED);
      motion = nothing;
      blocker_night = false;
    ...
    else if (((x <= 20) or (y <= 20)) and (blocker_motion_y == false))
    {
